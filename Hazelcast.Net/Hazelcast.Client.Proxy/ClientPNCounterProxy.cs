...
namespace Hazelcast.Client.Proxy
{
    ...
    {
        internal static readonly HashSet<IMember> _emptyAddressList = new HashSet<IMember>();
        internal volatile IMember _currentTargetReplicaAddress;
        ...
        public ClientPNCounterProxy(string serviceName, string name, HazelcastClient client) : base(serviceName, name, client)
        {
        ...
        internal ClientMessage InvokeAddInternal(long delta, bool getBeforeUpdate, HashSet<IMember> excludedAddresses,
            Exception lastException, IMember target)
        {
            if (target == null)
            {
            ...
            try
            {
                var request = PNCounterAddCodec.EncodeRequest(Name, delta, getBeforeUpdate, _observedClock.EntrySet(),
                    target.Uuid);
                return InvokeOnTarget(request, target.Uuid);
            }
            catch (Exception ex)
            {
                Logger.GetLogger(GetType()).Finest("Unable to provide session guarantees when sending operations to " +
                                                   target.ToString() + ", choosing different target. Cause: " +
                                                   ex.ToString());

                // Make sure that this only affects the local variable of the method
                if (excludedAddresses == _emptyAddressList)
                    excludedAddresses = new HashSet<IMember>();
        ...
        /// <returns>the result of the request invocation on a replica</returns>
        /// <exception cref="NoDataMemberInClusterException">if there are no replicas and the lastException is null</exception>
        internal ClientMessage InvokeGetInternal(HashSet<IMember> excludedAddresses, Exception lastException,
            IMember target)
        {
            if (target == null)
            {
            ...
            try
            {
                var request = PNCounterGetCodec.EncodeRequest(Name, _observedClock.EntrySet(), target.Uuid);
                return InvokeOnTarget(request, target.Uuid);
            }
            catch (Exception ex)
            {
        ...
        private IMember GetCRDTOperationTarget(HashSet<IMember> excludedAddresses)
        {
        ...
        private IMember ChooseTargetReplica(HashSet<IMember> excludedAddresses)
        {
        ...
        private List<IMember> GetReplicaAddresses(HashSet<IMember> excludedAddresses)
        {
            var dataMembers = Client.ClusterService.DataMemberList.ToList(); 
            ...
            var replicaAddresses = dataMembers
                ...
                .Take(currentReplicaCount).ToList();
        ...
        private int GetMaxConfiguredReplicaCount()
        {
            ...
            var request = PNCounterGetConfiguredReplicaCountCodec.EncodeRequest(Name);
        ...
        public void UpdateObservedReplicaTimestamps(IList<KeyValuePair<Guid, long>> timeStamps)
        {
