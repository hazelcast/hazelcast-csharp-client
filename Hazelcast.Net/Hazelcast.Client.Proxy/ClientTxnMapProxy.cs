...
namespace Hazelcast.Client.Proxy
{
	public class ClientTxnMapProxy<K, V> : ClientTxnProxy, ITransactionalMap<K, V>
	{
		...
		{
			var request = new TxnMapRequest<K,V>(GetName(), AbstractTxnMapRequest.TxnMapRequestType.ContainsKey, ToData(key));
		...
		{
            var request = new TxnMapRequest<K, V>(GetName(), AbstractTxnMapRequest.TxnMapRequestType.GetForUpdate, ToData(key));
			return Invoke<V>(request);
		}
		...
		{
            var request = new TxnMapRequest<K, V>(GetName(), AbstractTxnMapRequest.TxnMapRequestType.Put, ToData(key), ToData(value));
			return Invoke<V>(request);
		}

		public virtual void Set(K key, V value)
		{
            var request = new TxnMapRequest<K, V>(GetName(), AbstractTxnMapRequest.TxnMapRequestType.Set, ToData(key), ToData(value));
            Invoke<V>(request);
		}

		public virtual V PutIfAbsent(K key, V value)
		{
			var request = new TxnMapRequest<K,V>(GetName(), AbstractTxnMapRequest.TxnMapRequestType.PutIfAbsent, ToData(key), ToData(value));
            return Invoke<V>(request);
		}

		public virtual V Replace(K key, V value)
		{
			var request = new TxnMapRequest<K,V>(GetName(), AbstractTxnMapRequest.TxnMapRequestType.Replace, ToData(key), ToData(value));
            return Invoke<V>(request);
		}

		public virtual bool Replace(K key, V oldValue, V newValue)
		{
			var request = new TxnMapRequest<K,V>(GetName(), AbstractTxnMapRequest.TxnMapRequestType.ReplaceIfSame, ToData(key), ToData(oldValue), ToData(newValue));
			bool result = Invoke<bool>(request);
			return result;
		}
		...
		{
			var request = new TxnMapRequest<K,V>(GetName(), AbstractTxnMapRequest.TxnMapRequestType.RemoveIfSame, ToData(key), ToData(value));
			bool result = Invoke<bool>(request);
			return result;
		}
		...
		public virtual ICollection<K> KeySet()
		{
            ...
            MapKeySet result = Invoke<MapKeySet>(request);
		...
		public virtual ICollection<V> Values()
		{
            ...
            MapValueCollection result = Invoke<MapValueCollection>(request);
