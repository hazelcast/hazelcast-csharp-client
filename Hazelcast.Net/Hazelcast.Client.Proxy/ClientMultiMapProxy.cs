...
namespace Hazelcast.Client.Proxy
{
	public class ClientMultiMapProxy<K, V> : ClientProxy, IMultiMap<K, V>
	{
		...
		public virtual ICollection<V> Get(K key)
		{
            ...
            PortableCollection result = Invoke<PortableCollection>(request, keyData);
		...
		public virtual ICollection<K> KeySet()
		{
            ...
            PortableCollection result = Invoke<PortableCollection>(request);
		...
		public virtual ICollection<KeyValuePair<K, V>> EntrySet()
		{
            ...
            var dataEntrySet = result.GetEntrySet();
		...
		public virtual int ValueCount(K key)
		{
			...
			int result = Invoke<int>(request, keyData);
			return result;
		}
		...
		{
			Data keyData = GetSerializationService().ToData(key);
			MultiMapIsLockedRequest request = new MultiMapIsLockedRequest(keyData, name);
			bool result = Invoke<bool>(request, keyData);
			return result;
		}
		...
		{
			Data keyData = GetSerializationService().ToData(key);
			MultiMapLockRequest request = new MultiMapLockRequest(keyData, ThreadUtil.GetThreadId(), long.MaxValue, GetTimeInMillis(time, timeunit), name);
			bool result = Invoke<bool>(request, keyData);
			return result;
		}
	    ...
	    public void OnEntryEvent(PortableEntryEvent _event, bool includeValue, IEntryListener<K, V> listener)
	    {
		    ...
		    K key = (K)GetSerializationService().ToObject(_event.GetKey());
