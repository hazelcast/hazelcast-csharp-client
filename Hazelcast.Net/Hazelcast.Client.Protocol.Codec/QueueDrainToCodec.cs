...
{
	internal sealed class QueueDrainToCodec
	{
		public static readonly QueueMessageType RequestType = QueueMessageType.QueueDrainto;
		...
		{
			int requiredDataSize = QueueDrainToCodec.RequestParameters.CalculateDataSize(name);
		...
		{
			QueueDrainToCodec.RequestParameters parameters = new QueueDrainToCodec.RequestParameters();
			string name;
			name = null;
		...
		{
			int requiredDataSize = QueueDrainToCodec.ResponseParameters.CalculateDataSize(list);
			ClientMessage clientMessage = ClientMessage.CreateForEncode(requiredDataSize);
		...
		{
			QueueDrainToCodec.ResponseParameters parameters = new QueueDrainToCodec.ResponseParameters();
			IList<IData> list;
			list = null;
