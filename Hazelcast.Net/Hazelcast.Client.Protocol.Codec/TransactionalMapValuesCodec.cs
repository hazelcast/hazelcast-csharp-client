...
{
	internal sealed class TransactionalMapValuesCodec
	{
		public static readonly TransactionalMapMessageType RequestType = TransactionalMapMessageType.TransactionalmapValues;
		...
		{
			int requiredDataSize = TransactionalMapValuesCodec.RequestParameters.CalculateDataSize(name, txnId, threadId);
		...
		{
			TransactionalMapValuesCodec.RequestParameters parameters = new TransactionalMapValuesCodec.RequestParameters();
			string name;
			name = null;
		...
		{
			int requiredDataSize = TransactionalMapValuesCodec.ResponseParameters.CalculateDataSize(list);
			ClientMessage clientMessage = ClientMessage.CreateForEncode(requiredDataSize);
		...
		{
			TransactionalMapValuesCodec.ResponseParameters parameters = new TransactionalMapValuesCodec.ResponseParameters();
			IList<IData> list;
			list = null;
