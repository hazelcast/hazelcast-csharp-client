...
namespace Hazelcast.Client.Protocol.Codec
{
	internal sealed class QueueContainsAllCodec
	{
		...
		{
			int requiredDataSize = QueueContainsAllCodec.RequestParameters.CalculateDataSize(name, dataList);
		...
		{
			QueueContainsAllCodec.RequestParameters parameters = new QueueContainsAllCodec.RequestParameters();
			string name;
			name = null;
		...
		{
			int requiredDataSize = QueueContainsAllCodec.ResponseParameters.CalculateDataSize(response);
			ClientMessage clientMessage = ClientMessage.CreateForEncode(requiredDataSize);
		...
		{
			QueueContainsAllCodec.ResponseParameters parameters = new QueueContainsAllCodec.ResponseParameters();
			bool response;
