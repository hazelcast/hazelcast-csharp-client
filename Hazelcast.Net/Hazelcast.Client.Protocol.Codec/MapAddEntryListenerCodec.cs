...
namespace Hazelcast.Client.Protocol.Codec
{
	internal sealed class MapAddEntryListenerCodec
	{
		...
		{
			int requiredDataSize = MapAddEntryListenerCodec.RequestParameters.CalculateDataSize(name, includeValue);
		...
		{
			MapAddEntryListenerCodec.RequestParameters parameters = new MapAddEntryListenerCodec.RequestParameters();
			string name;
			name = null;
		...
		{
			int requiredDataSize = MapAddEntryListenerCodec.ResponseParameters.CalculateDataSize(response);
			ClientMessage clientMessage = ClientMessage.CreateForEncode(requiredDataSize);
		...
		{
			MapAddEntryListenerCodec.ResponseParameters parameters = new MapAddEntryListenerCodec.ResponseParameters();
			string response;
			response = null;
		...
		public class AbstractEventHandler
		{
			public static void Handle(ClientMessage clientMessage, HandleDelegate handle)
			{
                ...
                Logger.GetLogger(typeof(MapAddEntryListenerCodec)).Warning("Unknown message type received on event handler :" + clientMessage.GetMessageType());
			}
