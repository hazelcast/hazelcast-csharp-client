...
public final class MapIntegerLongCodec {
    ...
    private const int ENTRY_SIZE_IN_BYTES = IntSizeInBytes + LONG_SIZE_IN_BYTES;
    ...
    public static void Encode(ClientMessage clientMessage, Collection<Map.Entry<Integer, Long>> collection) {
        ...
        for (int i = 0; i < itemCount; i++) {
            ...
            EncodeLong(frame.content, i * ENTRY_SIZE_IN_BYTES + IntSizeInBytes, entry.getValue());
        }
        clientMessage.add(frame);
    }

    public static List<Map.Entry<Integer, Long>> Decode(ref ClientMessage.FrameIterator iterator) {
        ...
        for (int i = 0; i < itemCount; i++) {
            ...
            long value = DecodeLong(frame.content, i * ENTRY_SIZE_IN_BYTES + IntSizeInBytes);
