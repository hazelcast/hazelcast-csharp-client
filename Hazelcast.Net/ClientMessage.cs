...
namespace Hazelcast.Client.Protocol
{
	...
	/// </pre>
	/// </remarks>
	public class ClientMessage : MessageFlyweight, SocketWritable, SocketReadable
	{
		/// <summary>Current protocol version</summary>
		public const short Version = 0;

		/// <summary>Begin Flag</summary>
		public const short BeginFlag = unchecked((int)(0x80));

		/// <summary>End Flag</summary>
		public const short EndFlag = unchecked((int)(0x40));

		/// <summary>Begin and End Flags</summary>
		public const short BeginAndEndFlags = (short)(BeginFlag | EndFlag);

		/// <summary>Listener Event Flag</summary>
		public const short ListenerEventFlag = unchecked((int)(0x01));

		/// <summary>ClientMessage Fixed Header size in bytes</summary>
		public static readonly int HeaderSize;

		private const string PropHazelcastProtocolUnsafe = "hazelcast.protocol.unsafe.enabled";

		private static readonly bool UseUnsafe = bool.GetBoolean(PropHazelcastProtocolUnsafe);
		...
		public static Hazelcast.Client.Protocol.ClientMessage Create()
		{
			Hazelcast.Client.Protocol.ClientMessage clientMessage = new Hazelcast.Client.Protocol.ClientMessage();
			if (UseUnsafe)
			{
				clientMessage.Wrap(new UnsafeBuffer(new byte[InitialBufferSize]), 0);
			}
			else
			{
		...
		public static Hazelcast.Client.Protocol.ClientMessage CreateForEncode(int initialCapacity)
		{
		...
		public static Hazelcast.Client.Protocol.ClientMessage CreateForEncode(ClientProtocolBuffer buffer, int offset)
		{
			Hazelcast.Client.Protocol.ClientMessage clientMessage = new Hazelcast.Client.Protocol.ClientMessage();
		...
		public static Hazelcast.Client.Protocol.ClientMessage CreateForDecode(ClientProtocolBuffer buffer, int offset)
		{
			Hazelcast.Client.Protocol.ClientMessage clientMessage = new Hazelcast.Client.Protocol.ClientMessage();
		...
		protected internal virtual void WrapForEncode(ClientProtocolBuffer buffer, int offset)
		{
		...
		private void EnsureHeaderSize(int offset, int length)
		{
			...
			{
				throw new IndexOutOfRangeException("ClientMessage buffer must contain at least " + HeaderSize + " bytes! length: " + length + ", offset: " + offset);
			}
		}

		protected internal virtual void WrapForDecode(ClientProtocolBuffer buffer, int offset)
		{
			EnsureHeaderSize(offset, buffer.Capacity());
			base.Wrap(buffer, offset);
			Index(GetDataOffset());
		...
		public virtual Hazelcast.Client.Protocol.ClientMessage SetVersion(short version)
		{
		...
		public virtual Hazelcast.Client.Protocol.ClientMessage AddFlag(short flags)
		{
		...
		public virtual Hazelcast.Client.Protocol.ClientMessage SetMessageType(int type)
		{
		...
		public virtual Hazelcast.Client.Protocol.ClientMessage SetFrameLength(int length)
		{
		...
		public virtual Hazelcast.Client.Protocol.ClientMessage SetCorrelationId(int correlationId)
		{
		...
		public virtual Hazelcast.Client.Protocol.ClientMessage SetPartitionId(int partitionId)
		{
		...
		public virtual Hazelcast.Client.Protocol.ClientMessage SetDataOffset(int dataOffset)
		{
			Uint16Put(DataOffsetFieldOffset, dataOffset);
			return this;
		}

		public virtual Hazelcast.Client.Protocol.ClientMessage UpdateFrameLength()
		{
		...
		private int Accumulate(ByteBuffer byteBuffer, int length)
		{
			...
			if (readLength > 0)
			{
				int requiredCapacity = Index() + readLength;
				EnsureCapacity(requiredCapacity);
				buffer.PutBytes(Index(), ((byte[])byteBuffer.Array()), byteBuffer.Position(), readLength);
		...
		public override string ToString()
		{
			...
			if (len >= HeaderSize)
			{
				...
				sb.Append(", messageType=").Append(Hazelcast.Net.Ext.Extensions.ToHexString(GetMessageType()));
