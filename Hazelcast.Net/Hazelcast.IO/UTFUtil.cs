...
namespace Hazelcast.IO
{
	...
	public sealed class UTFUtil
	{
		...
		private static void WriteShortUTF(IDataOutput output, string str)
		{
			...
			for (; i_1 < stringLen; i_1++)
			{
				...
				else
				{
					if (c > unchecked((int)(0x07FF)))
					{
						byteArray[count++] = unchecked((byte)(unchecked((int)(0xE0)) | ((c >> 12) & unchecked((int)(0x0F)))));
						byteArray[count++] = unchecked((byte)(unchecked((int)(0x80)) | ((c >> 6) & unchecked((int)(0x3F)))));
						byteArray[count++] = unchecked((byte)(unchecked((int)(0x80)) | ((c) & unchecked((int)(0x3F)))));
					}
					else
					{
						byteArray[count++] = unchecked((byte)(unchecked((int)(0xC0)) | ((c >> 6) & unchecked((int)(0x1F)))));
						byteArray[count++] = unchecked((byte)(unchecked((int)(0x80)) | ((c) & unchecked((int)(0x3F)))));
					}
				}
			}
		...
		private static string ReadShortUTF(IDataInput input)
		{
			...
			while (count < utflen)
			{
				c = bytearr[count] & unchecked((int)(0xff));
				if (c > 127)
				{
					break;
				}
			...
			while (count < utflen)
			{
				...
				switch (c >> 4)
				{
					...
					case 13:
					{
						...
						chararr[chararr_count++] = (char)(((c & unchecked((int)(0x1F))) << 6) | (char2 & unchecked((int)(0x3F))));
						break;
					}
					...
					case 14:
					{
						...
						if (((char2 & unchecked((int)(0xC0))) != unchecked((int)(0x80))) || ((char3 & unchecked((int)(0xC0))) != unchecked((int)(0x80))))
						{
                            throw new UriFormatException("malformed input around byte " + (count - 1));
						}
						chararr[chararr_count++] = (char)(((c & unchecked((int)(0x0F))) << 12) | ((char2 & unchecked((int)(0x3F))) << 6) | ((char3 & unchecked((int)(0x3F))) << 0));
						break;
					}
