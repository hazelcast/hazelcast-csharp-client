...
namespace Hazelcast.Eventing
{
    ...
    public sealed class MixedEvent<TEventArgs>
    {
        ...
        private readonly Dictionary<Action<TEventArgs>, Func<TEventArgs, ValueTask>> _map = new Dictionary<Action<TEventArgs>, Func<TEventArgs, ValueTask>>();
        ...
        public void Add(Action<TEventArgs> method)
        {
            ...
            lock (_methods) _methods.Add(_map[method] = args =>
            {
                method(args);
                return default;
            });
        ...
        public static MixedEventHandler<TEventArgs> operator +(MixedEventHandler<TEventArgs> handler, Action<TEventArgs> method)
        {
            handler = handler ?? new MixedEventHandler<TEventArgs>();
            handler.Add(method);
            return handler;
        }
        */
        ...
        /// <param name="method">The event handling method.</param>
        /// <returns>The original event handler, with the specified event handling method added.</returns>
        public static MixedEventHandler<TEventArgs> operator +(MixedEventHandler<TEventArgs> handler, Func<TEventArgs, ValueTask> method)
        {
            handler = handler ?? new MixedEventHandler<TEventArgs>();
            handler.Add(method);
            return handler;
        ...
        */
        ...
        public void Remove(Action<TEventArgs> method)
        {
            lock (_methods)
            {
                if (_map.TryGetValue(method, out var x))
                {
                    _map.Remove(method);
                    _methods.Remove(x);
                }
