...
. Monitor the client on Hazelcast Management Center
...
== Step 6. Monitor with Management Center

Management Center is a user interface for managing and monitoring Hazelcast clusters.

In this step, you start a local instance of Management Center and use it to view the cluster and clients you've created in this tutorial.
...
----
docker run \
    -p 8080:8080 hazelcast/management-center:{server-full-version}
----
. In a web browser, go to localhost:8080 and enable "dev" mode.
. Enter `dev` as the cluster name and IP address/port of the server you've started in Step 2.
. Click *View Cluster* and go to *Clients*. You should see that three clients are connected to your cluster member: Management Center and your .NET clients.
. Go to *Storage* > *Maps*.
+
You should see that you have a map called `personnel-map` with six entries (keys and values). You'll also see the total amount of memory that those entries are taking up in your cluster.
