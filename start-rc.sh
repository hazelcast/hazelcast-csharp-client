...
HAZELCAST_VERSION="3.7.4-SNAPSHOT"
...
mvn dependency:get -DrepoUrl=${RELEASE_REPO} -Dartifact=com.hazelcast:hazelcast:${HAZELCAST_VERSION}:jar:tests -Ddest=hazelcast-${HAZELCAST_VERSION}-tests.jar

CLASSPATH="hazelcast-remote-controller-${HAZELCAST_RC_VERSION}.jar:hazelcast-${HAZELCAST_VERSION}.jar:hazelcast-${HAZELCAST_VERSION}-tests.jar:test/javaclasses"
...
if [ -n "${HAZELCAST_ENTERPRISE_KEY}" ]; then
    ...
    CLASSPATH="hazelcast-enterprise-${HAZELCAST_VERSION}.jar:"${CLASSPATH}
