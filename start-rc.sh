...
HAZELCAST_TEST_VERSION="3.7.5"
HAZELCAST_VERSION="3.7.5"
HAZELCAST_ENTERPRISE_VERSION="3.7.5"
...
mvn dependency:get -DrepoUrl=${RELEASE_REPO} -Dartifact=com.hazelcast:hazelcast:${HAZELCAST_TEST_VERSION}:jar:tests -Ddest=hazelcast-${HAZELCAST_TEST_VERSION}-tests.jar
...
if [ -n "${HAZELCAST_ENTERPRISE_KEY}" ]; then
    mvn dependency:get -DrepoUrl=${ENTERPRISE_RELEASE_REPO} -Dartifact=com.hazelcast:hazelcast-enterprise:${HAZELCAST_ENTERPRISE_VERSION} -Ddest=hazelcast-enterprise-${HAZELCAST_ENTERPRISE_VERSION}.jar
