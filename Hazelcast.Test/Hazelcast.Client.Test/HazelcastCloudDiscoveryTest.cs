    ...
    public class HazelcastCloudDiscoveryTest
    {
        ...
        public void Setup()
        {
            _httpListener.BeginGetContext(asyncResult =>
            {
                ...
                byte[] buffer;
                if (_responses.TryGetValue(token, out var jsonText))
                {
        ...
        public void TestParseResponse_DefaultPort()
        {
            ...
            Assert.IsTrue(nodes.TryGetValue(new Address("100.96.5.1", 31115), out var a1));
            Assert.IsTrue(a1 == new Address("10.113.44.139", 31115));
            Assert.IsTrue(nodes.TryGetValue(new Address("100.96.4.2", 31115), out var a2));
        ...
        public void TestParseResponse_DifferentPort()
        {
            ...
            Assert.IsTrue(nodes.TryGetValue(new Address("100.96.5.1", 5701), out var a1));
            Assert.IsTrue(a1 == new Address("10.113.44.139", 31115));
            Assert.IsTrue(nodes.TryGetValue(new Address("100.96.4.2", 5701), out var a2));
            Assert.IsTrue(a2 == new Address("10.113.44.130", 31115));
        }
    }
}