...
if($enterprise){
	...
	$classpath += "hazelcast-enterprise-${hazelcastEnterpriseVersion}.jar"
} else{
...
$nunitConsolePath=".\packages\NUnit.ConsoleRunner.3.8.0\tools\nunit3-console.exe"
...
if (!$netcore) {
	...
	if($coverage) {
		$dotCoverCmd=".\packages\JetBrains.dotCover.CommandLineTools.2018.2.0\tools\dotCover.exe"
