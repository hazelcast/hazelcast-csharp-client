...
{
    internal sealed class ReadOnlyLazyDictionary2<TKey, TValue> : IReadOnlyDictionary<TKey, IReadOnlyList<TValue>>
    {
        private readonly ISerializationService _serializationService;

        private readonly Dictionary<IData, ReadOnlyLazyEntry2<TKey, TValue>> _entries
            = new Dictionary<IData, ReadOnlyLazyEntry2<TKey, TValue>>();

        private readonly Dictionary<TKey, ReadOnlyLazyEntry2<TKey, TValue>> _keyEntries
            = new Dictionary<TKey, ReadOnlyLazyEntry2<TKey, TValue>>();


        /// <summary>
        /// Initializes a new instance of the <see cref="ReadOnlyLazyDictionary2{TKey,TValue}"/> class.
        /// </summary>
        /// <param name="serializationService">The serialization service.</param>
        public ReadOnlyLazyDictionary2(ISerializationService serializationService)
        {
        ...
        public void Add(IEnumerable<KeyValuePair<IData, IData>> entries)
        {
            foreach (var (keyData, valueObject) in entries)
            {
                if (!_entries.TryGetValue(keyData, out var entry))
                    _entries.Add(keyData, entry = new ReadOnlyLazyEntry2<TKey, TValue>(keyData, new ReadOnlyLazyList<TValue>(_serializationService)));
        ...
        private void EnsureKey(ReadOnlyLazyEntry2<TKey, TValue> entry)
        {
