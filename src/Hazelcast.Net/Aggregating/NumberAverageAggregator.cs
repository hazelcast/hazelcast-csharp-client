...
namespace Hazelcast.Aggregating
{
    ...
    /// </remarks>
    internal sealed class NumberAverageAggregator : AggregatorBase<double>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="NumberAverageAggregator"/> class.
        /// </summary>
        ...
        {
            output.Write(0D); // member side field not used on client
            output.Write(0L); // member side field not used on client
        }

        /// <inheritdoc />
        public override int ClassId => AggregatorDataSerializerHook.NumberAvg;
    }
}
